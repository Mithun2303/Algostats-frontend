...
export const BatchLeaderboard = () => {
    ...
    const getBatchLeaderboard = async () => {
        if (batchId != null) {
            const resp = await axios.get(`http://localhost:3000/api/leaderboard/batch/${batchId.toUpperCase()}`);
    ...
    return (
        <main className="p-12 bg-black">
            <div className="text-3xl text-white font-bold p-4 md:mx-24">
                <span>Leaderboard - Applied Mathematics and Computational Sciences - {batchId.toUpperCase()}</span>
            </div>
