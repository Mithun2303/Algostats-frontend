...
function StreamLeaderboard() {
    ...
    const getStreamLeaderboard = async () => {
        if (streamId != null) {
            const resp = await axios.get(`http://localhost:3000/api/leaderboard/stream/${streamId.toUpperCase()}`);
    ...
    const renderRow = (elt, idx) => {
        return (<TableRow className={`${idx % 2 == 0 ? " hover:bg-[#363636]" : "hover:bg-[#2b2b2b]"}  `}>
            ...
            <TableCell className="text-left">{elt.name}</TableCell>
    ...
    return (
        <main className="p-12">
            <div className="text-3xl text-white font-bold p-4 md:mx-24">
                <span>Leaderboard - Applied Mathematics and Computational Sciences - {streamId.toUpperCase()}</span>
            </div>
                ...
                <Table className="table-auto h-[10%]  w-full text-center">
                    <TableHeader>
                        <TableRow className="bg-[#001C2A] p-4">
                            ...
                            <TableHead className="px-4 text-white  font-bold text-lg text-left">Name</TableHead>
